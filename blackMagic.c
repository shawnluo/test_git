...
void array_vs_pointers(void) {
    ...
    printf("mat[0]: %zu\n", mat[0]);      //mat[0]是首行数组元素的地址
    //00000000
    printf("mat[0]: %zu\n", &mat[0]);      //&mat[0]是整个首行数组的地址
    //00000000

    printf("mat[0] + 1: %zu\n", mat[0] + 1);      //mat[0] + 1是第一行数组，偏移了一个元素后的地址
    //00000004
    printf("&mat[0] + 1: %zu\n", &mat[0] + 1);      //&mat[0] + 1是第一行数组偏移整行数组后的地址
    //000000020

    printf("*(&mat[0] + 1): %zu\n", *(&mat[0] + 1));    //不会打印5，只会打印第二行数组的首地址！！！
    //000000020

    printf("*(*(&mat[0] + 1): %zu\n", *(*(&mat[0] + 1)));    //打印第二行数组的第一个元素5！！！
    //5
    ...
    printf("mat + 1: %zu\n", mat + 1);      //mat + 1是第二行数组元素的地址
    //00000020

    //3.
    int **p = &mat[0][0];   //这个赋值是完全错误的！！！p指向一个地址的地址。而&a[0][0]是一个元素的地址
