...
for line in filtered_lines:
    ...
    if match_camdev and match_diff and match_now and match_ts:  # 增加对 timestamp 的匹配
        ...
        if camdev_number in camdev_data:
            ...
            timestamps[camdev_number].append(ts_value_ns)  # 存储 timestamp
...
# Plot 3: Timestamp Plot for Cameras (使用散点图，展示时间戳)
fig_ts, axs_ts = plt.subplots(6, 1, figsize=(16, 12), sharex=True)

for camdev_number in range(0, 6):
    if timestamps[camdev_number]:
        # 计算与第一个时间戳的差值
        first_timestamp = timestamps[camdev_number][0]
        relative_time = [(ts - first_timestamp) for ts in timestamps[camdev_number]]  # 保持纳秒单位
...
# 设置x轴范围和刻度
max_time = max(max(ts) for ts in timestamps.values() if ts)  # 找到最大的时间戳
