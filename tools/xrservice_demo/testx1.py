...
for camdev_number in range(0, 6):
    if timestamps[camdev_number]:
        ...
        time_in_seconds = [(t - start_time_ns) / 1e9 for t in timestamps[camdev_number]]  # convert to seconds
...
# Plot 3: Timestamp Plot for Cameras (使用散点图)
for camdev_number in range(0, 6):
    if timestamps[camdev_number]:
        time_stamps_in_seconds = [(t - timestamps[camdev_number][0]) / 1e9 for t in timestamps[camdev_number]]
        axs[2].scatter(time_stamps_in_seconds, [ts / 1e9 for ts in timestamps[camdev_number]], label=camdev_map[camdev_number])

axs[2].set_title('Timestamps for Cameras')
axs[2].set_xlabel('Time (seconds)')
axs[2].set_ylabel('Timestamp (in seconds)')
axs[2].legend()
